<script>
    ...
    import { createScaleMatrix, createRotateMatrix } from "~/math/transform";
...
void main(){
    ...
    function draw() {
        // 生成旋转矩阵
        let rotateMatrix = createRotateMatrix(normalize([1, 1, 1]), -30);
        // let scaleMatrix = createScaleMatrix(scale,normalize([1, 1, 1]));
        let scaleMatrix = createScaleMatrix(scale);

        let martrix = mult(rotateMatrix, scaleMatrix);
    ...
    function setControlGui() {
        ...
        let gui = new GUI();
        gui.add(controls, "scale", 0.5, 1.5);
    }
